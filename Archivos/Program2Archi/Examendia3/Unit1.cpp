...
{
	RegAlumno reg;
	RegFecha regf;     Cardinal p;
	pfi->read((char *)&regf,sizeof(regf));
	if(pfi->eof())
	{
...
void __fastcall TForm1::Button4Click(TObject *Sender)
{
  ...
  RegFecha regf;
  pf=new fstream(nomArch.c_str(),ios::in|ios::binary);
  pfi=new fstream("fecha.idx",ios::in|ios::binary);
  if (!pfi->fail()) {
...
{
    RegAlumno reg;
	 RegFecha regf;  //Crear el archivo indice por codigo
	 fstream fd(nomArch.c_str(),ios::in|ios::binary);
	 fstream fi("fecha.idx",ios::out|ios::binary);
	 if(!fd.fail())
	 {
		 while(!fd.eof())
		 {
			regf.pos=fd.tellg();//Devuelve la posicion del puntero
			fd.read((char *)&reg,sizeof(reg));
			if(!fd.eof())
			{
				regf.fecha=reg.fecha;
				fi.write((char*)&regf,sizeof(regf));
			}
		 }
	 }
	 fd.close();
...
void __fastcall TForm1::VerificarMenor1Click(TObject *Sender)
{
